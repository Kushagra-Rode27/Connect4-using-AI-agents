...
# Local libs
from connect4.players.ai2 import AIPlayer
from connect4.players.ai import AIPlayer as AIPlayer2
...
class Game:
    def __init__(self, player1, player2, time: int, board_init: np.array, m: int, n: int, popout_moves: int):
        ...
        self.state = board_init, {1: Integer(
            popout_moves), 2: Integer(popout_moves)}
        self.gui_board = []
        ...
        for j in range(n):
            ...
            for i in range(m):
                col = i * 50
                c = board[i][j]
                column.append(self.c.create_oval(
                    row, col, row + 50, col + 50, fill=self.colors[c]))
    ...
    def threaded_function(self, arg):
        ...
        for i in range(arg):
            ...
            if self.game_over:
                with open('logs.txt', 'w') as log_file:
                    s = 'Game Over\n'
                    print("AI as Player 1: " + str((get_pts(1, self.state[0]) - get_pts(
                        2, self.state[0]))*100 / get_pts(1, self.state[0])))
                    print("AI as Player 2: " + str((get_pts(2, self.state[0]) - get_pts(
                        1, self.state[0]))*100 / get_pts(2, self.state[0])))
    ...
    def make_move(self):
        ...
        valid_actions = get_valid_actions(
            current_player.player_number, self.state)
        ...
        if not self.game_over:
            if current_player.type == 'ai':
                ...
                try:
                    ...
                    p = mp.Process(target=turn_worker, args=(
                        self.state, send_end, p_func))
                    p.start()
            ...
            if action == TimeLimitExceedAction:
                log_action = {'player': current_player.player_number,
                              'move': 'TLE', 'is_pop': 'TLE'}
            elif action not in valid_actions:
                # Invalid move by the player. Don't do anything
                log_action = {'player': current_player.player_number,
                              'move': 'invalid', 'is_pop': 'invalid'}
            else:
                move, is_popout = action
                self.update_board(
                    int(move), current_player.player_number, is_popout=is_popout)
                log_action = {'player': current_player.player_number,
                              'move': int(move), 'is_pop': is_popout}
            ...
            self.current.configure(
                text=f'cur: {self.players[self.current_turn].player_string}, ')
    ...
    def update_board(self, column: int, player_num: int, is_popout: bool = False):
        ...
        if not is_popout:
            if 0 in board[:, column]:
                for row in range(1, board.shape[0]):
                    ...
                    if update_row >= 0:
                        ...
                        self.c.itemconfig(
                            self.gui_board[column][update_row], fill=self.colors[self.current_turn + 1])
                        break
            else:
                err = 'Invalid move by player {}. Column {}'.format(
                    player_num, column, is_popout)
                raise Exception(err)
        else:
            if 1 in board[:, column] or 2 in board[:, column]:
                ...
                board[0, column] = 0
                self.c.itemconfig(
                    self.gui_board[column][0], fill=self.colors[0])
            else:
                err = 'Invalid move by player {}. Column {}'.format(
                    player_num, column)
                raise Exception(err)
...
def main(player1: str, player2: str, init_fine_name: str, time: int):
    ...
    m, n = board.shape
    Game(make_player(player1, 1), make_player(
        player2, 2), time, board, m, n, num_pop_outs)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "start_file", help="Initial state of the game (Num popout moves, Board)")
    # Giving Max of 50 seconds to AI agent in each step
