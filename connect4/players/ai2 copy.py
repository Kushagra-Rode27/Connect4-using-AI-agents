...
# TODO: Add weights to pop-out moves due to limited number of moves
# TODO: Dynamically update weights based on game state
# TODO: Brainstorm why minimax takes more time than expectimax even with alpha-beta pruning
...
class AIPlayer:
    ...
    def get_total_moves(self, state):
        # return len(state[0])*len(state[0][0])//2 + 2 * ((state[1][self.player_number]).get_int())
    ...
    def getExpectimaxUtility(self, board, num_popouts):
        ...
        # if (num_popouts[3 - self.player_number].get_int() == 0):
        #     return self.get_weight_quad((board, num_popouts), 1, 2, 1)
        # if (num_popouts[self.player_number].get_int() == 0):
        #     return (num1 - self.get_weight_fast((board, num_popouts), 0, 3)*(num2))
        # return (num1 - self.get_weight_fast((board, num_popouts), 0, 2)*(num2))
        # return (num1 - self.get_weight_linear((board, num_popouts), 1, 2)*num2)
        return (num1 - (0.75 + 3*(num_popouts[self.player_number].get_int()/20))*num2)
        # return (num1 - self.get_weight_inc((board, num_popouts), 0, 1, 0)*num2)
        # return (num1 - ((num2*num2))/(num1+0.1))
    ...
    def get_dynamic_weight(self, state):
        ...
        # total_moves = self.get_total_moves(state)
        # move_number = self.get_move_number(state)
        # if move_number < total_moves//3:
        # return num1 - 0.75*num2
        # return -1*num2
        # return (num1 - (2*(num2*num2))/(num1+0.1))
        return (num1 - (num2*num2)/(num1+0.1))

        # return (2*(num1*num1))/(num2+0.1) - num2
    ...
    def get_intelligent_move(self, state: Tuple[np.array, Dict[int, Integer]]) -> Tuple[int, bool]:
        ...
        def minimax(state, depth, start_time, isMax):
            best_action = None
            board, popouts = state
        ...
        max_action, max_cost = None, -np.inf
        iterations = 2
        while (iterations > 0 and time.time() - start_time < self.time - self.buffer):
            max_action_extra, max_cost_extra = minimax(
                state, depth, start_time, True)
            if (max_cost_extra > max_cost):
                max_action = max_action_extra
                max_cost = max_cost_extra
            depth += 2
            iterations -= 1
        ...
        #     depth += 1
        #     iterations -= 1
        print(self.player_string + " : " + str(max_action))
        end_time = time.time()
    ...
    def get_expectimax_move(self, state: Tuple[np.array, Dict[int, Integer]]) -> Tuple[int, bool]:
        ...
        orig_depth, max_action = 3, None
        if (state[1][self.player_number].get_int() >= 9 or (state[1][self.player_number].get_int() <= 2 and state[1][self.player_number].get_int() > 0)):
            orig_depth = 2
        # max_action, max_cost = expectimax(state, depth, start_time)
        # if (state[0].shape[1] == 10) and (self.time - self.buffer >= 7):
        #     orig_depth = 6

        def expectimax(state, depth, start_time):
            depth -= 1
            my_actions = get_valid_actions(self.player_number, state)
            board, popOuts = state
            max_cost, max_action = -np.inf, None
            if len(my_actions) == 0 or time.time() - start_time >= self.time - self.buffer:
                return max_action, self.getExpectimaxUtility(board, popOuts)
            while len(my_actions) > 0:
                col, is_popOut = random.choice(my_actions)
                move_cost = -np.inf
                mat = board.copy()
                pops = popOuts.copy()
                popped = self.get_next_state(
                    col, is_popOut, mat, pops, self.player_number)
                opp_actions = get_valid_actions(
                    3-self.player_number, (mat, pops))

                move_cost = self.getExpectimaxUtility(mat, pops)

                # if (depth == orig_depth):
                #     move_cost += self.get_weight_quad(0, 50, 1) * \
                #         pops(self.player_number).get_int()

                if time.time() - start_time >= self.time - self.buffer:
                    if (move_cost > max_cost):
                        max_cost = move_cost
                        max_action = (col, is_popOut)
                    if popped:
                        pops[self.player_number].increment()
                    break

                if (depth != 0 and len(opp_actions) != 0):
                    depth -= 1
                    opp_move_cost = 0
                    explored_children = 0
                    while len(opp_actions) > 0:
                        new_col, new_is_popOut = random.choice(opp_actions)
                        explored_children += 1
                        new_mat = mat.copy()
                        new_pops = pops.copy()
                        new_popped = self.get_next_state(
                            new_col, new_is_popOut, new_mat, new_pops, 3 - self.player_number)

                        my_new_actions = get_valid_actions(
                            self.player_number, (new_mat, new_pops))

                        if time.time() - start_time >= self.time - self.buffer:
                            opp_move_cost = self.getExpectimaxUtility(
                                new_mat, new_pops)
                            if new_popped:
                                new_pops[3-self.player_number].increment()
                            break

                        if depth == 0 or len(my_new_actions) == 0:
                            opp_move_cost = self.getExpectimaxUtility(
                                new_mat, new_pops)
                        else:
                            _, child_cost = expectimax(
                                (new_mat, new_pops), depth, start_time)
                            opp_move_cost += child_cost
                        ...
                        if time.time() - start_time >= self.time - self.buffer:
                            break
                        opp_actions.remove((new_col, new_is_popOut))
                    move_cost = opp_move_cost // explored_children
                if (move_cost > max_cost):
                    max_cost = move_cost
                    max_action = (col, is_popOut)
                if popped:
                ...
                if time.time() - start_time >= self.time - self.buffer:
                    break
                my_actions.remove((col, is_popOut))
            return max_action, max_cost

        # Do the rest of your implementation here
        start_time = time.time()
        max_action, _ = expectimax(state, orig_depth, start_time)
        # max_action, max_cost = None, -np.inf
        # depth = 2
        # iterations = 2
        # while (iterations > 0 and time.time() - start_time < self.time - self.buffer):
        #     max_action_extra, max_cost_extra = expectimax(
        #         state, depth, start_time)
        #     max_cost_extra -= depth
        #     if (max_cost_extra >= max_cost):
        #         max_action = max_action_extra
        #         max_cost = max_cost_extra
        #     depth += 2
